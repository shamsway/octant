...
```bash
grep -rn '"Postgres"\|"Restic"\|"Registry"\|"Backblaze"\|"Nautobot"\|"Cloudflare"\|"OpenAI API Key"\|"Anthropic API Key"\|"Replicate API Key"\|"Openrouter API Key"\|"Cohere API Key"\|"Groq API Key"\|"Langfuse API Key"\|"n8n.shamsway.net"\|"n8n_admin"\|"nautobot_db"\|"postgres_litellm"\|"postgres_n8n"\|"postgres_langfuse"\|"postgres_phoenix"' roles/ scripts/ terraform/
```
