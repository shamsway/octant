...
# Example: lab.octant.net, lab.example.com
...
- Replace hardcoded `Host(\`excalidraw.example.com\`)` with `Host(\`${servicename}.${domain}\`)`
...
- Templatize all hardcoded values
- Replace `Host(\`web.example.com\`)` with `Host(\`${servicename}.${domain}\`)`
...
- Nomad job: templatize dns servers (currently hardcoded), fix email from `pgadmin@example.com` to use variable
...
- Nomad job: IMPORTANT - templatize all hardcoded `example.com` and `consul.example.com:8500` references
  - Replace `consul.example.com:8500` with `consul.service.consul:8500` (internal Consul address)
  - Replace all `example.com` host rules with `${servicename}.${domain}`
...
- Nomad job: templatize hardcoded `loki.example.com` and dns servers
...
- Nomad job: templatize hardcoded `grafana.example.com` and dns servers
...
- Remove `backend "consul"` block
- Change defaults
- `config.yaml`: update `uptimekuma.example.com` references and SSL check URLs to use the correct domain (or remove SSL checks for now since Tier 1 is HTTP-only)
...
- Remove `backend "consul"` block
- Change defaults
- Update `webui_url` default from `https://chatllm.example.com` to match domain pattern
...
If `lab.example.com` is configured:
- Verify Cloudflare wildcard record exists
- Verify Traefik obtains Let's Encrypt cert
- Verify `grafana.lab.example.com` resolves and serves HTTPS
