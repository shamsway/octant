...
variable "builderip" { 
  default = "192.168.252.118"
}

source "vmware-iso" "vmware" {
  vm_name              = "octantnode"
  output_directory     = "${vmware-iso.vmware.vm_name}-output"
  format               = "ovf"
...
/*   provisioner "shell" {
  ...
  provisioner "shell" {
    ...
    scripts          = [
      "cp /tmp/cloud-init.cfg /etc/cloud/cloud.cfg.d/99-octant.cfg
  ...
  post-processor "shell-local" {
    environment_vars = [
      "APPLIANCE_NAME=${vmware-iso.vmware.vm_name}",
      "APPLIANCE_VERSION=1",
      "APPLIANCE_OVA=${vmware-iso.vmware.vm_name}"
    ]
    inline           = [
